...
<div class='main'>
    ...
    <div class="content">
        <ul class="sort">
            ...
            <li class="sort_item" v-for="item in indexSortList" :key="item._id" @mouseover="pullList(item.sortId)"
    ...
    <div class="search">
        <input class="search_text" v-model="searchWord" type="text"><div @click="searchGood" class="search_btn"></div>
    </div>
</div>
...
.down_item{position: relative;width:100%}
.sub_sortInfo1{position: absolute;z-index: 99;visibility:hidden;width: 100%;
transition: all .4s ease;background-color: white;border-top:1px solid #ccc;}
.sub_sortInfo{display: flex;height: 0px;width: 90%;flex-wrap: wrap;margin:0 auto;position: absolute;
z-index: 99;visibility:hidden;transition: all .4s ease;background-color: white;
border-top:1px solid #ccc;overflow: hidden;}
.sub_sort_item{width: 200px;}
.active{visibility: visible;height:210px;}
