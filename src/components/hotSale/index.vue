...
<div class='main'>
    <div class="left_body">
        ...
        <ul class="goodslist">
            <li class="goods_item" @mouseenter="sub_goodslist1(item.bigSort)" @mouseleave="hide_goodslist"
            v-for="item in bigSortList" :key="item._id"><div class="item_content">{{item.bigSort}}</div></li>          
        </ul>       
    </div>
...
.left_body{flex:1;height: 500px;background-color: darkslategray;}
.goodslist{display: flex;flex-direction: column;height: 100%;}
.goods_item{flex:1;width: 100%;display: flex;align-items: center;}
.item_content{width: 80%;margin: 0 auto;display: flex;justify-content: space-between;color:white}
.goods_item:hover{background-color:chocolate;}
.right_body{flex:3;height: 500px;border:1px solid #ccc;position: relative;}
.sub_goodslist{height: 100%;background-color: crimson;position:absolute;visibility: hidden;
width:0px;overflow: hidden;transition: all .5s ease-in-out;}
.item_ul{display:flex;flex-direction: column;flex-wrap:wrap;}
.item_li{width:200px;height: 50px;line-height: 50px;}
